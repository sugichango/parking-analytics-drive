...
- `data/updated_integrated_data_FY[年度].csv.gz` がソースファイルです。
- 新しい年度のデータを追加する場合は、ファイル名を `updated_integrated_data_FY2026.csv.gz` のように命名して `data` フォルダに配置してください。
- ダッシュボード（Mode ①）はフォルダ内のファイルを自動検出し、サイドバーに年度選択ボタンを表示します。
- デフォルトでは `2025` 年度を表示するように設定されています。
- （圧縮せずに `.csv` のまま置きたい場合は、`unified_dashboard.py` のコードを修正する必要がありますが、メモリ効率のため `.csv.gz` を推奨します）
