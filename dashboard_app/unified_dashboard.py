...
if "①" in mode:
    ...
    if available_files:
        ...
        selected_year_d1 = st.sidebar.selectbox("分析対象年度 (一般利用)", years, index=default_index, key="d1_year_select")
    ...
    if os.path.exists(file_path):
        ...
        st.sidebar.markdown("---")
        if st.sidebar.button("🔄 キャッシュをクリアして更新", key="clear_cache_d1_final"):
            st.cache_data.clear()
            st.rerun()
