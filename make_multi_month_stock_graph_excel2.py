...
def add_capacity_to_excel_charts(output_dir: str, parking_key: str) -> None:
    """
    完成したグラフ付きExcelに収容台数ライン(100%・90%・70%)を追加する。
    ...
    for fname in os.listdir(output_dir):
        ...
        try:
            ref_str = first_series.val.numRef.f  # NumRef.f に "data!$P$9:$P$245" のような文字列
        except Exception as e:
        ...
        for ws in graph_sheets:
            ...
            for chart in charts:
                ...
                # ★ グラフサイズを大きくする（ここを追加）★
                try:
                    # 単位は「Excelの列幅・行高」的な相対的なサイズです。
                    # 数値はお好みで調整してください。
                    chart.width = 40   # 横方向を大きめに
                    chart.height = 25 # 縦方向を大きめに
                except Exception:
                    pass
...
def main():
    ...
    for key, paths in sorted(files_by_key.items()):
        ...
        # 既存の関数で、グラフ付きExcelを作成
        # output_dir は期間ごとのルート配下に作る
        output_dir = os.path.join(output_root_dir, safe_key)
        ...
        # ★ グラフ作成後に収容台数ライン(100/90/70%)を追加 ★
        add_capacity_to_excel_charts(output_dir=output_dir, parking_key=key)
