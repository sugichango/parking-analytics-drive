...
(1) ① 一般利用台数推移分析
    ...
    【年度間比較分析】
    「📊 年度間比較分析を表示する」チェックボックスをクリックすると表示されます。
    ※クリック後、データ読み込みに少々時間がかかります。

    ・年度合計KPI：各年度の総利用台数・現金収入と前年比（+/-表示）を表示します。
...
--------------------------------------------------
更新日: 2026年5月7日（年度間比較機能・前年比表示改善対応）
開発協力: Antigravity
--------------------------------------------------
"""
    with st.expander("📖 使い方マニュアルを表示"):
        st.text(_MANUAL_TEXT)
        st.download_button(
            label="📥 マニュアルをテキストで保存",
            data=_MANUAL_TEXT.encode("utf-8"),
